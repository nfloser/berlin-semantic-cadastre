...
public sealed class WfsGeoJsonParser
{
    ...
    {
        if (string.IsNullOrWhiteSpace(json)) throw new ArgumentException("WFS response must not be blank.", nameof(json));
        ...
        foreach (JsonElement element in features.EnumerateArray())
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(id)) throw new InvalidDataException("Feature has no stable identifier.");
                ...
                Geometry geometry = _reader.Read<Geometry>(geometryElement.GetRawText());
                geometry.SRID = sourceSrid;
                if (geometry.IsEmpty) throw new InvalidDataException("Feature geometry is empty.");
                if (!geometry.IsValid) throw new InvalidDataException("Feature geometry is topologically invalid.");
    ...
    private static IReadOnlyDictionary<string, string?> ReadProperties(JsonElement feature)
    {
        ...
        if (!feature.TryGetProperty("properties", out JsonElement properties) || properties.ValueKind != JsonValueKind.Object) return result;
